#!/bin/bash
#this script was modified of Daniel Utter #https://dutter.github.io/projects/oral_metapan#recruiting-hmp-metagenomes-to-the-contigs
...
mkdir hmp_all_$site
cd hmp_all_$site
find ../../vic_met/results/trim/trim -name "*val*fq.*" -exec ln -s {} . ';'
cd ../
########
...
ls --color=none hmp_all_$site/*_1_val_1.fq.gz | split -l 8 -d -a 3 - batches_$site/$site-
...
r2=\$(echo \"\$FQ\" | sed 's/_1_val_1.fq.gz/_2_val_2.fq.gz/')

bowtie2 -x $prefix -1 \$FQ -2 \$r2 --no-unal --threads 8 | samtools view -b - | samtools sort -@ 12 - > \$(echo \"\$FQ\" | sed 's/hmp_all_$site/bt_mapped_${prefix}_$site/; s/\_1_val_1.fq.*$/.bam/')

# index as next anvio step requires indexed bams
samtools index \$(echo \"\$FQ\" | sed 's/hmp_all_$site/bt_mapped_${prefix}_$site/; s/\_1_val_1.fq.*$/.bam/')
